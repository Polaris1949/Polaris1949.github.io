...
| `a_uniq.emplace(args)` | `pair<iterator, bool>` | *要求：*`value_type`从`args`到`a`中应是`EmplaceConstructible`。 !!! *效果：* 插入由`std::forward<Args>(args)...`构造的`value_type`对象`t`，如果且仅如果容器中没有元素有和`t`键等价的键。返回的双对的`bool`成员是`true`，如果且仅如果本次插入发生；且双对的`iterator`成员指向有和`t`键等价的键的元素。 | 对数 |
